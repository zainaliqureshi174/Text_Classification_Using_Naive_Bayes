...
def generate_folds(X, Y, nfolds=4):
    """
    Split the training data into stratified-nfolds...
    ...
    for cidx, c in enumerate(classes):
        idx = Y == c  # find class example indices
        Yt = Y[idx]  # get class labels
        Xt = X[idx, :]  # get training examples
        nexamples = Xt.shape[0]
        
        # Generate a random permutation of the indices
        ridx = np.arange(nexamples)  # generate indices
        np.random.shuffle(ridx)  # shuffle randomly ...
        
        # number of test examples per fold, ensure it's an integer
        nexamples = int(nexamples / nfolds)
        sridx = set(ridx)  # create a set of all indices
        sidx = 0
        
        for k in range(nfolds):
            testidx = ridx[sidx:sidx + nexamples]  # use integer slicing
            trainidx = list(sridx.difference(testidx))  # set difference
            sidx += nexamples
